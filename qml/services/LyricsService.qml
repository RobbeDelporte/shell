...
Singleton {
    ...
    function parseLrc(text) {
        ...
        const creditKeywords = ["作词", "作曲", "编曲", "制作", "收录", "演奏", "词：", "曲：", "Lyricist", "Composer", "Arranger", "Producer", "Mixing", "Mastering"];
    ...
    function fetchNetEaseLyrics(id, reqId) {
        ...
        Requests.get(url, text => {
            ...
            const res = JSON.parse(text);
            if (res.lrc?.lyric) {
                updateModel(parseLrc(res.lrc.lyric));
                loading = false;
            }
        });
    }
